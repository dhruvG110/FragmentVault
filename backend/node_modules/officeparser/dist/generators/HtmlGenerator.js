...
<body>
    ...
    renderMetaTags() {
        ...
        if (m.created)
            tags.push(`<meta name="dcterms.created" content="${new Date(m.created).toISOString()}">`);
        if (m.modified)
            tags.push(`<meta name="dcterms.modified" content="${new Date(m.modified).toISOString()}">`);
        if (m.lastModifiedBy)
...
<script>
    (function() {
        ...
        if (document.readyState === 'complete') tryInit();
            ...
            case 'break':
                return node.metadata?.breakType === 'page' ? '<hr class="page-break">' : '<br>';
            case 'code': {
                const meta = node.metadata;
            ...
            case 'paragraph':
            case 'heading': {
                const tag = node.type === 'heading' ? `h${node.metadata?.level || 1}` : 'p';
    ...
    getInlineStyles(node) {
        ...
        if (node.metadata) {
            const meta = node.metadata;
            if (meta.alignment)
                styles.push(`text-align: ${meta.alignment}`);
            if (meta.backgroundColor)
                styles.push(`background-color: ${meta.backgroundColor}`);
            if (meta.verticalAlign)
                styles.push(`vertical-align: ${meta.verticalAlign}`);
        ...
        if (node.formatting) {
            const f = node.formatting;
            if (f.color)
                styles.push(`color: ${f.color}`);
            if (f.backgroundColor)
                styles.push(`background-color: ${f.backgroundColor}`);
            if (f.size)
                styles.push(`font-size: ${f.size}`);
            if (f.font)
                styles.push(`font-family: ${f.font}, sans-serif`);
        }
        return styles.join('; ');
    }
